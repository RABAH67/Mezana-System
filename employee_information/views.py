...
from employee_information.models import TYPEOFVISASTATUS, Department, Position, Employees
...
def employees(request):
    page_obj = Employees.objects.all()
    
    totalmada = Employees.objects.filter(position_id=3 ).aggregate(Sum('totalmadakhil'))['totalmadakhil__sum']
    ...
    if item_name != '' and item_name is not None:
        ...
        page_obj = Employees.objects.filter(
            Q(firstname__icontains=item_name)
            | 
            Q(lastname__icontains=item_name)            
            | 
            Q(gender__icontains=item_name)            
            | 
            Q(contact__icontains=item_name)
...
def save_employee(request):
    ...
    if len(check) > 0:
        ...
        try:
            ...
            if (data['id']).isnumeric() and int(data['id']) > 0 :
                save_employee = Employees.objects.filter(id = data['id']).update(code=data['code'], firstname = data['firstname'],lastname = data['lastname'],dob = data['dob'],gender = data['gender'],contact = data['contact'],department_id = dept,position_id = pos,date_hired = data['date_hired'],salary = data['salary'],status = data['status'])
            else:
                save_employee = Employees(code=data['code'], firstname = data['firstname'],lastname = data['lastname'],dob = data['dob'],gender = data['gender'],contact = data['contact'],department_id = dept,position_id = pos,date_hired = data['date_hired'],salary = data['salary'],status = data['status'])
                save_employee.save()
            resp['status'] = 'success'
        except Exception:
            resp['status'] = 'failed'
            print(Exception)
            print(json.dumps({"code":data['code'], "firstname" : data['firstname'],"lastname" : data['lastname'],"dob" : data['dob'],"gender" : data['gender'],"contact" : data['contact'],"email" : data['email'],"address" : data['address'],"department_id" : data['department_id'],"position_id" : data['position_id'],"date_hired" : data['date_hired'],"salary" : data['salary'],"status" : data['status']}))
    return HttpResponse(json.dumps(resp), content_type="application/json")
